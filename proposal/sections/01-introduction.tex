...
\begin{itemize}[leftmargin=*, itemsep=1pt, parsep=0pt, topsep=2pt]
    \item Longitudinal rate data with two time coordinates: Published datasets record snapshots, whereas forecasting rate movement needs repeated observation of the same stay date over successive days.
    \item Room comparability over time: OTA listings reorder room options between sessions and add partner rates asynchronously, so a rule such as ``take the cheapest room'' silently switches products and corrupts the series.
    \item A decision layer for independent operators: Existing prototypes target the traveller or the revenue manager of a large chain, not the competitive-set view an independent hotel needs.
    \item Vietnam-specific demand structure: National holidays, lunar-calendar events and city-level festivals create local demand signals that a generic calendar does not encode.
\end{itemize}
