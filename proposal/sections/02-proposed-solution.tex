...
A six-day pilot on 10 properties validated parsing, persistence, reference calibration and outcome classification. Before the labelled-data clock starts, the final 355-property workbook is audited and load is benchmarked at 100, 200 and then 355 properties with five check-in dates. Pilot and benchmark observations remain available for pipeline audit but are excluded from model development. Once the cohort and protocol are locked, the required daily load covers all 355 properties for six near-term future check-in dates, prioritising lead times from 1 to 30 days. If the measured daily capacity permits, a second operator-initiated run rotates two or three farther dates: 31--90-day dates every 3--7 days, 91--180-day dates weekly, and longer lead times every 2--4 weeks. Passed dates are closed and replaced by future dates. This design keeps the near-term series dense while allowing the farther-date load to contract before data quality is sacrificed.
...
\hline
Operation and deployment & Local scripts; VPS and Docker Compose planned & Current collection uses operator-initiated local runs; deployment packaging is added only after benchmark and integration gates pass \\
\hline
\end{tabular}
\end{table}
