...
_FIELDS: dict[str, dict[str, str]] = {
    ...
    "field_group": _d("Nhom field: room_identity, rate_plan, price, hotel_attributes, artifact_source_metadata.", source="queries._MISSINGNESS_FIELD_GROUPS", grain="field",
                      allowed="5 nhom"),
    "missing_kind": _d("Phan loai missing: structural_expected cho sentinel sold-out (khong co payload phong/gia); class_dependent cho field con lai - PHAI doc theo "
                       "`null_class` (required/optional/source-dependent), khong tu dong la bat thuong; structural_not_requested khi run khong yeu cau luu artifact; "
                       "unexpected_if_missing khi da yeu cau artifact ma thieu path.",
                       source="queries missingness/artifact rules", grain="field_value_cell hoac item group",
                       allowed="structural_expected/class_dependent/structural_not_requested/unexpected_if_missing", scope="MAIN"),
    ...
    "n_distinct_checkin_dates": _d("So ngay check-in (anchor) PHAN BIET trong nhom (thu / thang / co calendar), khong bi nhan theo so hotel/item/option: mot nhom co nhieu item "
                                   "co the chi la MOT ngay lap qua nhieu hotel va crawl day. Bang thu/holiday chi mo ta coverage cua cac anchor da chon, khong phai weekday effect.",
                                   source="derived / COUNT(DISTINCT checkin_date)", grain="nhom coverage", scope="MAIN"),
    ...
    "null_class": _d("Lop NULL cua field (registry null_taxonomy): required_contract = NULL la vi pham hop dong du lieu; optional_listing = Booking co the khong cong bo; "
                     ...
                     scope="MAIN/CONFIG"),
    "canonical_key_role": _d("Field co nam trong canonical key hay khong: room_identity_key (ten phong, suc chua, giuong, dien tich), rate_plan_key (breakfast, free_cancellation, "
                             "cancellation_policy) hoac none. NULL o field trong key doi ngu nghia key.", source="app.scraper.reference (room_identity_payload/rate_plan_key)",
                             grain="field", allowed="room_identity_key/rate_plan_key/none", scope="MAIN/CONFIG"),
    "rationale": _d("Ly do gan lop NULL cho field (dua tren hop dong du lieu CLAUDE.md muc 4.3 / hanh vi Booking).", source="null_taxonomy.py", grain="field", scope="CONFIG"),
    ...
    "divergent_price_groups": _d("So nhom trung key co tu 2 muc gia tro len (min < max) - KHONG phai dong trung byte.", source="derived", grain="nhom item x canonical key", scope="RAW/MAIN"),
    "divergent_share": _d("divergent_price_groups / duplicate_groups (mau so: nhom trung key).", source="derived", grain="nhom item x canonical key", unit="ty le (0-1)",
                          structural="NULL khi duplicate_groups = 0", scope="RAW/MAIN"),
    "duplicate_group_rate": _d("duplicate_groups / n_groups (mau so: tat ca nhom item x canonical key khong sold-out cua nhom dong).", source="derived", grain="nhom item x canonical key",
                               unit="ty le (0-1)", structural="NULL khi n_groups = 0", scope="RAW/MAIN"),
    "max_group_size": _d("So observation lon nhat cua mot nhom trung key trong nhom dong (0 neu khong co nhom trung).", source="derived", grain="nhom item x canonical key", scope="RAW/MAIN"),
    "spread_kind": _d("Loai do lech gia trong nhom khac gia: absolute_vnd = max - min (VND); relative_symmetric = (max - min) / ((max + min) / 2).", source="derived",
                      grain="nhom khac gia", allowed="absolute_vnd/relative_symmetric", scope="RAW/MAIN"),
    "spread_mean": _d("Trung binh do lech gia trong nhom khac gia (theo spread_kind).", source="derived", grain="nhom khac gia", scope="RAW/MAIN"),
    "spread_min": _d("Do lech gia nho nhat trong nhom khac gia (theo spread_kind).", source="derived", grain="nhom khac gia", scope="RAW/MAIN"),
    "spread_max": _d("Do lech gia lon nhat trong nhom khac gia (theo spread_kind).", source="derived", grain="nhom khac gia", scope="RAW/MAIN"),
    "spread_q25": _d("Phan vi 25% (noi suy tuyen tinh) cua do lech gia trong nhom khac gia (theo spread_kind).", source="derived", grain="nhom khac gia", scope="RAW/MAIN"),
    "spread_q50": _d("Trung vi do lech gia trong nhom khac gia (theo spread_kind).", source="derived", grain="nhom khac gia", scope="RAW/MAIN"),
    "spread_q75": _d("Phan vi 75% cua do lech gia trong nhom khac gia (theo spread_kind).", source="derived", grain="nhom khac gia", scope="RAW/MAIN"),
    "spread_q90": _d("Phan vi 90% cua do lech gia trong nhom khac gia (theo spread_kind).", source="derived", grain="nhom khac gia", scope="RAW/MAIN"),
    "spread_q95": _d("Phan vi 95% cua do lech gia trong nhom khac gia (theo spread_kind).", source="derived", grain="nhom khac gia", scope="RAW/MAIN"),
    "spread_q99": _d("Phan vi 99% cua do lech gia trong nhom khac gia (theo spread_kind).", source="derived", grain="nhom khac gia", scope="RAW/MAIN"),
    "room_key": _d("canonical_room_key cua nhom trung key (hex64, dau vao trung gian - khong publish rieng).", source="curated_observation_keys.canonical_room_key",
                   grain="nhom item x canonical key", unit="hex64", scope="RAW"),
    "rate_key": _d("canonical_rate_key cua nhom trung key (hex64, dau vao trung gian - khong publish rieng).", source="curated_observation_keys.canonical_rate_key",
                   grain="nhom item x canonical key", unit="hex64", scope="RAW"),
    "group_id": _d("Dinh danh nhom trung key trong audit sample: `<item_id>:<10 ky tu dau canonical_room_key>:<10 ky tu dau canonical_rate_key>`.", source="derived", grain="nhom item x canonical key", scope="RAW"),
    "audit_reasons": _d("Tieu chi chon nhom vao audit sample: `tieu_chi#hang` ngan cach `;` (largest_group_size / largest_absolute_spread / largest_relative_spread; hang tinh rieng trong tung nguon, moi hotel toi da 1 nhom moi "
                        "tieu chi; chon xac dinh, khong ngau nhien).", source="derived", grain="nhom item x canonical key", scope="RAW"),
    "group_size": _d("So observation (option) KHONG sold-out cua nhom item x canonical key (>= 2).", source="derived", grain="nhom item x canonical key", scope="RAW"),
    "group_min_price": _d("Gia/dem thap nhat trong nhom (VND).", source=_PRICE_SRC, grain="nhom item x canonical key", unit="VND", scope="RAW"),
    "group_max_price": _d("Gia/dem cao nhat trong nhom (VND).", source=_PRICE_SRC, grain="nhom item x canonical key", unit="VND", scope="RAW"),
    "spread_abs": _d("Do lech gia tuyet doi cua nhom = group_max_price - group_min_price (VND).", source="derived", grain="nhom item x canonical key", unit="VND", scope="RAW"),
    "spread_rel_symmetric": _d("Do lech gia doi xung cua nhom = (max - min) / ((max + min) / 2).", source="derived", grain="nhom item x canonical key", unit="ty le", scope="RAW"),
    "price_rank_in_group": _d("Thu tu gia tang dan trong nhom (1 = re nhat; bang gia thi theo thu tu room_option_index).", source="derived", grain="observation trong nhom", scope="RAW"),
    "room_option_index": _d("Thu tu rate-option trong bang Booking cua lan crawl (0-based); unique cung crawl_run_item_id.", source="price_observations.room_option_index", grain="observation", scope="RAW"),
    "observed_at": _d("Thoi diem cao observation.", source="price_observations.observed_at", grain="observation", unit="datetime", tz="UTC", scope="RAW"),
    "original_price": _d("Gia goc truoc giam (VND) - khong nam trong canonical key.", source="price_observations.original_price", grain="observation", unit="VND",
                         structural="NULL khi Booking khong hien gia goc", scope="RAW"),
    "discount_percent": _d("% giam - khong nam trong canonical key.", source="price_observations.discount_percent", grain="observation", unit="%",
                           structural="NULL khi khong hien giam gia", scope="RAW"),
    "rooms_left": _d("So phong con lai theo banner 'chi con N phong' - khong nam trong canonical key.", source="price_observations.rooms_left", grain="observation", unit="phong",
                     structural="NULL khi Booking khong hien banner", scope="RAW"),
    "taxes_fees": _d("So tien thue/phi tach rieng (VND) - khong nam trong canonical key.", source="price_observations.taxes_fees", grain="observation", unit="VND",
                     structural="NULL khi nguon chi noi da bao gom hoac khong tach rieng", scope="RAW"),
    "price_includes_tax": _d("Gia da gom thue (true) / chua gom (false) - khong nam trong canonical key.", source="price_observations.price_includes_tax", grain="observation",
                             unit="boolean", structural="NULL khi nguon khong noi ro", scope="RAW"),
    "room_type_raw": _d("Ten loai phong nguyen goc (cat toi da 200 ky tu) - canonical_room_key dung ban da chuan hoa.", source="price_observations.room_type_raw", grain="observation", scope="RAW"),
    "bed_config": _d("Cau hinh giuong nguyen van (cat toi da 200 ky tu).", source="price_observations.bed_config", grain="observation",
                     structural="NULL khi listing khong cong bo (optional_listing)", scope="RAW"),
    "room_area": _d("Dien tich phong theo nguyen van Booking.", source="price_observations.room_area", grain="observation", structural="NULL khi listing khong cong bo", scope="RAW"),
    "breakfast_included": _d("Bao gom bua sang (nam trong rate_plan_key).", source="price_observations.breakfast_included", grain="observation", unit="boolean", scope="RAW"),
    "free_cancellation": _d("Huy mien phi (nam trong rate_plan_key; NULL tao key rieng).", source="price_observations.free_cancellation", grain="observation", unit="boolean",
    ...
    "max_lead_time": _d("Lead-time lon nhat (ngay) cua item cua anchor.", source="derived", grain="checkin_date (anchor)", unit="ngay", scope="MAIN"),
    "is_public_holiday_any_city": _d("It nhat 1 thanh pho co ngay le vao ngay anchor (holiday_date = checkin_date).", source="holidays.checkin_calendar_flags", grain="checkin_date (anchor)",
                                    unit="boolean", scope="MAIN"),
    "is_tet_any_city": _d("It nhat 1 thanh pho co co Tet vao ngay anchor.", source="holidays.checkin_calendar_flags", grain="checkin_date (anchor)", unit="boolean", scope="MAIN"),
    "is_festival_period_any_city": _d("It nhat 1 thanh pho co festival vao ngay anchor.", source="holidays.checkin_calendar_flags", grain="checkin_date (anchor)", unit="boolean", scope="MAIN"),
    "is_major_event_any_city": _d("It nhat 1 thanh pho co su kien lon vao ngay anchor.", source="holidays.checkin_calendar_flags", grain="checkin_date (anchor)", unit="boolean", scope="MAIN"),
}
