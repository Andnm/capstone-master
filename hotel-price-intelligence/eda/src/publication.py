...
PUBLISHED_TABLES: dict[str, TableSpec] = dict([
    ...
    _t("collision_option_near_time_concentration", "derived:metrics.collision_near_time_concentration", "7.2", "RAW",
       "source pair x vn_crawl_date x hotel (shared option-pair chenh observed_at 0-5 phut)",
       "shared canonical option-pairs trong bucket 0-5 phut (share_of_near_time_pairs = ty le tren tong bucket)"),
    ...
    _t("run_day_operational_flags", "derived:metrics.daily_operational_anomaly_flags", "7.3", "PROTOCOL",
       "source-day PRODUCTION (source x vn_crawl_date, co >= 1 run is_protocol_run)",
       "n_items = item RAW cua source-day production; z-score tren phan phoi cac source-day production cua CHINH nguon (pilot/pre-protocol khong dinh hinh baseline)"),
    _t("run_day_operational_flags_raw_appendix", "derived:metrics.daily_operational_anomaly_flags", "7.3", "RAW",
       "source-day RAW (source x vn_crawl_date, ke ca pilot/pre-protocol) - PHU LUC",
       "n_items = item RAW cua source-day; z-score tren phan phoi CA pilot - chi de doi chieu, khong dung ket luan production"),
    ...
    _t("item_checkin_month_distribution_main", "derived:metrics.item_coverage_with_anchors", "7.4", "MAIN",
       "owned item -> thang check-in (kem so ngay anchor phan biet)", "owned terminal MAIN items; moi item dung 1 lan; n_distinct_checkin_dates = so anchor cua thang"),
    _t("item_checkin_weekday_distribution_main", "derived:metrics.item_coverage_with_anchors", "7.4", "MAIN",
       "owned item -> thu check-in (kem so ngay anchor phan biet + danh sach ngay)", "owned terminal MAIN items; moi item dung 1 lan; n_distinct_checkin_dates = so anchor cua thu do"),
    _t("checkin_anchor_dates_main", "derived:metrics.checkin_anchor_dates_table", "7.4", "MAIN",
       "checkin_date (anchor) - 1 dong / ngay check-in cua owned MAIN item", "owned terminal MAIN items trong anchor; tong so dong = so anchor phan biet"),
    ...
    _t("null_taxonomy_registry", "derived:null_taxonomy.registry_dataframe", "7.9", "CONFIG", "field (registry NULL taxonomy: 1 dong / field)", _NA),
    _t("missingness_null_taxonomy", "derived:metrics.null_taxonomy_by_source", "7.9", "MAIN",
       "field_value_cell -> source x field x null_class (gan nhan lop cho missingness_available_observations)",
       "n_total = observation available trong (source, field); ba lop phan hoach du tap dong"),
    _t("missingness_null_class_summary", "derived:metrics.null_class_summary", "7.9", "MAIN", "null_class (3 lop)",
       "n_total_cells = tong cell (source, field, observation) cua CHINH lop do (mau so rieng tung lop)"),
    _t("missingness_required_contract_by_field", "derived:metrics.required_field_null_stats", "7.9", "MAIN",
       "field required_contract (1 dong / field)", "n_total = observation available cua chinh field tren cac nguon con bi rang buoc (nguon mien tru bi loai)"),
    ...
    # ------------------------------------------------------------------ 7.11 data quality
    _t("duplicate_series_summary_by_source_city", "derived:metrics.duplicate_series_summary", "7.11", "RAW+MAIN",
       "scope x source x city (nhom item x canonical key trung; kem dong (all))",
       "n_groups = observation khong sold-out - observation du (moi nhom 1 observation goc); divergent_share tren duplicate_groups"),
    _t("duplicate_series_price_spread_summary", "derived:metrics.duplicate_series_price_spread_summary", "7.11", "RAW+MAIN",
       "scope x source x loai do lech (tuyet doi VND / doi xung tuong doi) cua nhom KHAC GIA", "n_divergent_groups = nhom trung key co >= 2 muc gia"),
    _t("duplicate_series_audit_sample", "derived:metrics.duplicate_series_audit_sample", "7.11", "RAW",
       "observation cua nhom khac gia duoc chon xac dinh (top group size / spread tuyet doi / spread tuong doi moi nguon)", _NA),
...
INTERMEDIATE_METRICS: frozenset[str] = frozenset({
    ...
    # File 17 M3: dau vao trung gian cua 3 bang duplicate_series_*
...
PUBLISHED_FIGURES: dict[str, FigureSpec] = {spec.name: spec for spec in (
    ...
    FigureSpec("run_duration_by_source", "7.3", ("run_duration_and_throughput",), "Phan bo thoi luong run production (is_protocol_run) theo nguon"),
    FigureSpec("finish_hour_distribution", "7.3", ("finish_hour_distribution",), "Gio VN hoan thanh run production theo nguon"),
    FigureSpec("active_hotel_by_date", "7.4", ("active_hotel_by_crawl_date_source", "cohort_attrition_by_version"),
               "Hotel active theo ngay crawl va nguon, kem chu thich cohort transition"),
    ...
    FigureSpec("checkin_coverage_weekday_month", "7.4", ("item_checkin_weekday_distribution_main", "item_checkin_month_distribution_main"),
               "Phan bo owned item theo thu va thang check-in, kem so ngay anchor phan biet tren moi cot"),
